...
static int simcom_wwan_bind(struct usbnet *dev, struct usb_interface *intf)
{
    ...
    dev_info(&intf->dev, "simcom usbnet bind here\n");
...
static struct sk_buff *simcom_wwan_tx_fixup(struct usbnet *dev, struct sk_buff *skb,
                    gfp_t flags)
{
...
static int simcom_wwan_rx_fixup(struct usbnet *dev, struct sk_buff *skb)
{
    ...
    case 0x40:
        dev_dbg(&dev->intf->dev, "packetv4 coming\n");
        proto = htons(ETH_P_IP);
        break;
    case 0x60:
        dev_dbg(&dev->intf->dev, "packetv6 coming\n");
        proto = htons(ETH_P_IPV6);
        break;
    case 0x00:
        dev_dbg(&dev->intf->dev, "packet coming\n");
